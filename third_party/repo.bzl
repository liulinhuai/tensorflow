...
def _cuda_http_archive_impl(repository_ctx):
    ...
    if cuda_version:
        ...
        sha256 = repository_ctx.attr.sha256_dict["{cuda_version}-{os}-{arch}".format(cuda_version = cuda_version, os = os, arch = arch)]

        # The format of the url_template should be the following:
        # https://developer.download.nvidia.com/compute/cuda/redist/cuda_nvtx/{os}-{arch}/cuda_nvtx-{os}-{arch}-{version}-archive.tar.xz
        url = repository_ctx.attr.url_template.format(os = os, arch = archive_arch_suffix, version = archive_version)
        ...
        if repository_ctx.attr.build_template:
            version = archive_version.split(".")[0]
        ...
        repository_ctx.file("version.txt", "")
